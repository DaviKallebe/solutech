...
//                        ChatSDK.logError(throwable);
    ...
    public void passwordLogin(String email, String password, Context context) {
        ...
        NM.auth().authenticateWithCurrentUser()
                .observeOn(AndroidSchedulers.mainThread())
                .doFinally(() -> {
                ...
                .subscribe(() -> {
                    AppBackgroundMonitor.shared().setEnabled(true);
                    InterfaceManager.shared().a.startMainActivity(context, extras);
                }, e -> {
    ...
    public void authenticateWithAppUser(FirebaseUser user, Context context, String name, String image) {
        if(authenticating) {
            return;
        }
        authenticating = true;

        Disposable disposable = NM.auth().authenticateWithAppUser(user)
                .observeOn(AndroidSchedulers.mainThread())
                .doFinally(() -> {
                    authenticating = false;
                })
                .subscribe(() -> {
                    ...
                    if (image != null)
                        currentUser.setAvatarURL(image);
    ...
    /* Dismiss dialog and open main context.*/
    protected void afterLoginAppUser(Context context) {
        AppBackgroundMonitor.shared().setEnabled(true);
